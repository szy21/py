...
for i in range(npert):
    ...
    for yri in range(nyr1):
        ...
        for moni in range(12):        
            ...
            dMSE[yri*12+moni,:,:] = (MSECol[-1,...]-MSECol[0,...])/(np.size(mind)*3*3600)
            eneCol = np.sum(MSE*dpMon,1)/GRAV
            dene[yri*12+moni,:,:] = (eneCol[-1,...]-eneCol[0,...])/(np.size(mind)*3*3600)
...
plt.plot(lat,np.cumsum(np.mean(np.mean(net-glb,0),-1)*xsc*sc*ysc*2.5))
...
m.drawcoastlines()
m.drawcountries()
m.contourf(x,y,np.mean(Totdivt+vertdivt+det,0),clev,cmap=cmap,extend='both')
cb = m.colorbar(location='bottom',size='5%',pad='8%')
plt.tight_layout()
plt.figure()
m.drawcoastlines()
m.drawcountries()
m.contourf(x,y,np.mean(uvdht+wdht+Trdivt+det,0),clev,cmap=cmap,extend='both')
cb = m.colorbar(location='bottom',size='5%',pad='8%')
plt.tight_layout()